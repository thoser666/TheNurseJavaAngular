...
public class UserService {
  ...
  public UserDto getUser(Long id) {
    ...
    User user =
        userOpt.orElseThrow(() -> new MyRuntimeException("User with id : " + id + " not found"));
  ...
  public void unfollow(Long idFollowing, Long idFollowed) {
    ...
    User userFollowing =
        userOptFollowing.orElseThrow(() -> new MyRuntimeException("User not found"));
    User userFollowed = userOptFollowed.orElseThrow(() -> new MyRuntimeException("User not found"));

    String username = SecurityContextHolder.getContext().getAuthentication().getName();
    if (!userFollowing.getUsername().equals(username)
        || userFollowed.getUsername().equals(username)) {
      throw new MyRuntimeException("Unfollowing not allowed");
    }
    Optional<Following> follOpt =
        followRepository.findById(new FollowingId(idFollowing, idFollowed));
  ...
  public List<UserDto> getAllSuggestedUsers() {
    List<User> notFollowing =
        userRepository.findByUserIdNotInAndIsEnabled(
            authService.getCurrentUser().getFollowing().stream()
  ...
  public Boolean assignRole(String username, String rolename) {
    Role role =
        roleRepository
            .findByName(rolename)
            .orElseThrow(() -> new MyRuntimeException(("Role not found")));
    User user =
        userRepository
            .findByUsername(username)
            .orElseThrow(() -> new MyRuntimeException("User not found"));
